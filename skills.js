var skills = load_data("dnd-sheet-skills", {});
...
skills_list.forEach(skills_entry => { get_ele(`skill-${skills_entry}`).onchange = () => {update_skills();} });
set_skills()
set_skill_mods();
update_skills();
...
function set_skills() {
    skills_list.forEach(skills_entry => {
        ...
        var skill_ele = get_ele(`skill-${skills_entry}`);
        if (skill_ele.type == "checkbox") { skill_ele.checked = value; }
        else { skill_ele.value = value; }
    });
}
...
function set_skill_mods() {
    skill_mod_list.forEach(skill_mod => {
        ...
        var skill_ele = get_ele(`skill-${skill_mod}`);
...
function update_skills() {
    skills_list.forEach(skills_entry => {
        var skill_ele = get_ele(`skill-${skills_entry}`);
        skills[skills_entry] = skill_ele.type == "checkbox" ? skill_ele.checked : skill_ele.value;
    });
    ...
    save_data("dnd-sheet-skills", skills);
}